# app.py - VERSIÓN COMPLETA CON CLICK EN SECCIONES, SUBCATEGORÍAS Y SISTEMA DE MENSAJES
...
# Inicializar gestores (sin PaymentManager)
@st.cache_resource
def init_managers():
    auth = AuthManager()
...
# Procesar redirección desde Dashboard
    if 'ir_a' in st.query_params and st.query_params['ir_a'] == 'mis_documentos':
        # Guardar la sección preseleccionada
        if 'seccion' in st.query_params:
            st.session_state['seccion_seleccionada_documentos'] = st.query_params['seccion']
        # Forzar la opción del menú a "Mis Documentos" antes de que el widget se cree
        st.session_state['menu_principal'] = "📁 Mis Documentos"
        # Limpiar parámetros para que no se queden en la URL
    ...
    with st.sidebar:
        ...
        if 'menu_principal' not in st.session_state:
            st.session_state['menu_principal'] = menu[0]  # primera opción por defecto
        
        # Si el valor actual no está en el menú (por ejemplo, después de logout), resetear
        ...
        # Mostrar secciones asignadas (para usuarios)
        if st.session_state['rol'] != 'master':
    ...
    # ============================================
    # MIS DOCUMENTOS - VISTA MEJORADA CON CATEGORÍAS Y BUSCADOR (CORREGIDO)
    # ============================================
        ...
        if not secciones_usuario:
            ...
            # Redirección desde Dashboard (para preseleccionar la sección)
            ...
            # Selector de sección (con preselección)
            ...
            # Mostrar botones de categorías (con scroll horizontal)
            ...
            # Inicializar archivos_personales por si no hay (para evitar NameError)
            archivos_personales = []
                    ...
                    if st.session_state['rol'] == 'master':
                        with col3:
                            with st.popover("✏️ Editar", use_container_width=True):
                                ...
                                nueva_desc = st.text_area(
                                    ...
                                    height=150,
                                    key=f"edit_desc_{pub['id']}_{i}"
                                )
                                col_ed1, col_ed2 = st.columns(2)
                                with col_ed1:
                                    if st.button("💾 Guardar", key=f"save_edit_{pub['id']}_{i}", use_container_width=True):
                                        ...
                                        if exito:
                                            ...
                                            st.error(msg)
                                with col_ed2:
                                    if st.button("❌ Cancelar", key=f"cancel_edit_{pub['id']}_{i}", use_container_width=True):
                                        st.rerun()
                        ...
                        with col4:
                            with st.popover("🗑️ Eliminar", use_container_width=True):
                                ...
                                with col_btn1:
                                    if st.button("✅ Sí, eliminar", key=f"confirm_delete_{pub['id']}_{i}", use_container_width=True):
                                        ...
                                        if exito:
                                            ...
                                            st.error(msg)
                                with col_btn2:
                                    if st.button("❌ Cancelar", key=f"cancel_delete_{pub['id']}_{i}", use_container_width=True):
                                        st.rerun()
                    st.divider()
            else:
                st.info("No hay publicaciones del master en esta categoría")
            
            # ========== FORMULARIO PARA PUBLICAR (solo master) ==========
            if st.session_state.get('show_publish_form', False) and st.session_state['rol'] == 'master':
                with st.form("form_publicar_personal"):
                    ...
                    # Usamos la variable archivos_personales ya definida (puede ser lista vacía)
    ...
    # ============================================
    # GESTIÓN DE USUARIOS (Master) - CON PUBLICACIÓN POR SUBCATEGORÍA
    # ============================================
        ...
        with tab2:
            ...
            if usuarios_lista:
                ...
                # Asegurar que secciones_actuales sea una lista (por si acaso)
                if not isinstance(secciones_actuales, list):
                    secciones_actuales = []

                # Mostrar checkboxes en 3 columnas
                cols = st.columns(3)
                nuevas_secciones = []
                ...
                for i, (seccion_id, seccion_info) in enumerate(SECCIONES.items()):
                    col_idx = i % 3
                    with cols[col_idx]:
                        # Usamos una clave única que incluya el usuario seleccionado y la sección
    ...
    # ============================================
    # MI PERFIL - VERSIÓN COMPLETA (SIN PLANES DE PAGO)
    # ============================================
        ...
        with tab_estadisticas:
            ...
            archivos_personales = storage_manager.listar_archivos_usuario(
                st.session_state['usuario'], incluir_publicaciones=False
            )
            publicaciones = storage_manager.obtener_publicaciones_usuario(
                st.session_state['usuario'], 
            ...
            if publicaciones:
                st.markdown("**Últimas publicaciones disponibles:**")
                for pub in publicaciones[:5]:
                    st.markdown(f"""
                    <div style="background: #e8f4fd; padding: 0.5rem; border-radius: 8px; margin: 0.25rem 0;">
                        📢 <strong>{pub['nombre_original']}</strong><br>
    ...
    # ============================================
        ...
        with col1:
            ...
            if st.button("🗑️ Limpiar Facturas", type="secondary", key="btn_limpiar"):
                ...
                if confirmar:
                    cols = ['NUMERO_FACTURA', 'CLIENTE', 'PRODUCTO', 'CANTIDAD', 'ESTADO', 'FECHA_CARGA', 'FECHA_ACTUALIZACION']
                    df_vacio = pd.DataFrame(columns=cols)
                    storage_manager.guardar_facturas(df_vacio)
        ...
        with col2:
            ...
            **Facturas:** {len(archivos_personales) if 'archivos_personales' in locals() else 0}
            """)
            
            st.markdown("### 🎨 Estados")
            for estado, color in storage_manager.colores_estado.items():
                st.markdown(f"<span style='background:{color}; color:white; padding:2px 8px; border-radius:4px; margin:2px; display:inline-block'>{estado}</span>", unsafe_allow_html=True)
